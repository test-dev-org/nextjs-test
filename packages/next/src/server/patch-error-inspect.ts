...
export function sourceMapStackTrace(
  stack: string,
  options: { colors?: boolean; showIgnored?: boolean } = {}
): string {
  const error = new Error()
  // Ensure the first line doesn't get added automatically by Error's constructor
  // We'll build our own stack string.
  error.name = 'Error'
  error.message = ''
  error.stack = `${error.name}: ${error.message}\n${stack}`
  return parseAndSourceMapWithOptions(error, {
    colors: options.colors ?? process.stdout.isTTY,
    showIgnored: options.showIgnored ?? false,
  })
}

function parseAndSourceMapWithOptions(
  error: Error,
  inspectOptions: util.InspectOptions & { showIgnored?: boolean }
): string {
  // Allow showing ignored frames when requested
  const showIgnoreListed = inspectOptions.showIgnored ?? false
  // We overwrote Error.prepareStackTrace earlier so error.stack is not sourcemapped.
  let unparsedStack = String(error.stack)
  // We could just read it from `error.stack`.
  // This works around cases where a 3rd party `Error.prepareStackTrace` implementation
