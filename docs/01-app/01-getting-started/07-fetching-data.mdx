...
Next.js wraps `fetch` with [`React.cache`](https://react.dev/reference/react/cache) so that identical `fetch` calls (`GET` and `HEAD` only) within the same render pass result in a single request.

In other words, you can call the same URL with the same options multiple times in the current render pass and only one request will be made. You can opt out of this behavior by using an `Abort` signal.

Deduplicating requests with [`React.cache`](https://react.dev/reference/react/cache) is a form of memoization, which is scoped to the current request — concurrent requests don’t share this cache.

You can also enable deduplication across components and future renders by setting `cache: 'force-cache'` in your `fetch` options.

This means that future calls to the same URL and options will also return the cached result.
