...
<AppOnly>
For polyfills that must load early, use `next/script` with the `beforeInteractive` strategy.

This can negatively impact users with modern browsers, so avoid it when possible.
...
export default function Layout(/* layout props */) {
  return (
    {/* other UI elements */}
    <Script strategy="beforeInteractive" src="https://source-to-the-polyfill/" />
    {/* A vendor script, that needs the polyfill, and also needs to run immediately */}
    <Script strategy="beforeInteractive" src="https://critical-vendor-script/" />
    {/* rest of UI */}
  )
}
```

For other cases, you can create a client module that loads the polyfills you need.

```tsx filename="app/polyfill.ts"
'use client'
import languageFeature from 'polyfill-for-feature'

if (typeof window !== 'undefined') {
  // check if the polyfill needs to be applied
}

export const Polyfill = null
```

And use it in your root layout.
...
Using this strategy prevents dead-code elimination from removing the polyfill and marks its usage explicitly.

The best approach is to isolate unsupported features to specific UI sections and conditionally load the polyfill before using them.
...
export const useAnalytics = () => {
  const tracker = useCallback(async (data: unknown) => {
    if (!('structuredClone' in globalThis)) {
      globalThis.structuredClone = await import('polyfills/structured-clone')
    }
