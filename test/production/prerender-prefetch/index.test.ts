    ...
    if (optimisticClientCache) {
      it('should attempt cache update on link hover/touch start', async () => {
        ...
        await retry(async () => {
          ...
          expect(requests.some((url) => url.includes('/blog/first.json'))).toBe(
            true
          )
        })
      })
      ...
      it('should not attempt client cache update on link hover/touch start', async () => {
        ...
        await retry(async () => {
          ...
          expect(cacheKeys.some((url) => url.includes('/blog/first'))).toBe(
            true
          )
          expect(cacheKeys.some((url) => url.includes('/blog/second'))).toBe(
            true
          )
        })
