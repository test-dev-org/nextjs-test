...
describe('parallel-routes-and-interception', () => {
  ...
  describe('parallel routes', () => {
    it('should support parallel route tab bars', async () => {
      ...
      const hasViewsHome = async () => {
        await retry(async () => {
          expect(await browser.waitForElementByCss('#views-home').text()).toBe(
            'Views home'
          )
        })
      }
      ...
      const hasImpressions = async () => {
        await retry(async () => {
          expect(await browser.waitForElementByCss('#impressions').text()).toBe(
            'Impressions'
          )
        })
      }
      ...
      const hasSubscribers = async () => {
        await retry(async () => {
          expect(await browser.waitForElementByCss('#subscribers').text()).toBe(
            'Subscribers'
          )
        })
      }
    ...
    it('should throw a 404 when no matching parallel route is found', async () => {
      ...
      await browser.elementByCss('#view-duration-link').click()
      await retry(async () => {
        expect(await browser.waitForElementByCss('#view-duration').text()).toBe(
          'View duration'
        )
      })
    ...
    it('should render nested parallel routes', async () => {
      ...
      await retry(async () => {
        expect(await browser.waitForElementByCss('#nested-main').text()).toBe(
          'Nested page'
        )
      })
      ...
      await retry(async () => {
        expect(await browser.waitForElementByCss('#sidebar-main').text()).toBe(
          'root sidebar here'
        )
      })
    })
    ...
    it('should only scroll to the parallel route that was navigated to', async () => {
      ...
      await retry(async () => {
        expect(await browser.eval('window.scrollY')).toBe(position)
      })
    })
    ...
    it('should apply the catch-all route to the parallel route if no matching route is found', async () => {
      ...
      await retry(async () => {
        expect(await browser.waitForElementByCss('#slot-content').text()).toBe(
          'slot catchall'
        )
      })
      ...
      await retry(async () => {
        expect(await browser.waitForElementByCss('#main').text()).toBe('foo')
      })
      await retry(async () => {
        expect(await browser.waitForElementByCss('#slot-content').text()).toBe(
          'foo slot'
    ...
    it('should match the catch-all routes of the more specific path, if there is more than one catch-all route', async () => {
      ...
      await browser
        ...
        .click()
      await retry(async () => {
        expect(await browser.waitForElementByCss('#main').text()).toBe('bar')
      })
      await retry(async () => {
        expect(await browser.waitForElementByCss('#slot-content').text()).toBe(
          'slot catchall'
      ...
      await browser
        ...
        .click()
      await retry(async () => {
        expect(await browser.waitForElementByCss('#main').text()).toBe('foo id')
      })
      await retry(async () => {
        expect(await browser.waitForElementByCss('#slot-content').text()).toBe(
          'foo id catchAll'
  ...
  describe('route intercepting', () => {
    ...
    it('should render an intercepted route from a slot', async () => {
      ...
      await retry(async () => {
        expect(await browser.waitForElementByCss('#default-slot').text()).toBe(
          'default from @slot'
        )
      })
      ...
      await retry(async () => {
        expect(
          await browser
            ...
            .text()
        ).toBe('interception from @slot/nested')
      })
    ...
    it('should re-render the layout on the server when it had a default child route', async () => {
      ...
      await retry(async () => {
        expect(await browser.elementByCss('#children-foo').text()).toBe('foo')
      })
    })
    ...
    it('should support intercepting local dynamic sibling routes', async () => {
      ...
      await retry(async () => {
        expect(await browser.waitForElementByCss('#main-slot').text()).toBe('1')
      })
    })
